...
const AddModal = ({ setModalOpen, modalOpen, trigger }) => {
    ...
    useEffect(() => {
        const clickHandler = (event) => {
            ...
            // Close if clicked outside the modal and the trigger (if it exists)
            if (
                modalOpen &&
    ...
    return (
        <>
            <div className="container mx-auto py-10">
                <div
                    className={`fixed left-0 top-0 flex h-full min-h-screen w-full items-center justify-center bg-dark/90 px-4 py-5 modal ${modalOpen ? "open" : ""
                        }`}
                >
                    <div
                        ref={modal} className={`modal ${modalOpen ? 'open' : ''}`}
